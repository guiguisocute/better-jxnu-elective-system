...
“评价管理 → 人机验证”统一管理 Turnstile / Cap。全站一次只能选择一个 provider，但「评价提交」「举报提交」与「学号查询」可分别开关；切换 provider 会保留另一套凭据。Cap Standalone 与 Valkey 的部署说明见 [`cap/README.md`](cap/README.md)，公网 API 固定走 `https://getxk.betterjxnu.cn/cap`。
...
```bash
curl https://getxk.betterjxnu.cn/healthz
curl https://getxk.betterjxnu.cn/api/config
curl -H 'Origin: https://test.better-jxnu-elective-system.pages.dev' \
  -I https://getxk.betterjxnu.cn/api/enrollments
```
