...
func TestBuildStudentRecordSelectedTerm(t *testing.T) {
	...
	built := BuildStudentRecord("999999999999", agg, ctx, "")
...
// that term finalized. In the regression case, 教务 had already moved its selected
// term to 26-27第1学期, so 25-26第2学期 looked "在读" even though it was finished.
func TestBuildStudentRecordFinalizedTermCountsFinishedSemester(t *testing.T) {
	...
	if got := BuildStudentRecord("999999999999", agg, ctx, "").Row["total_earned"]; got != float64(3) {
		t.Fatalf("without a finalized term, earned = %v, want 3", got)
	}
	// 声明 25-26第2学期 已结束 → T1+T2 计入，未来学期仍不算。
	if got := BuildStudentRecord("999999999999", agg, ctx, "25-26第2学期").Row["total_earned"]; got != float64(7) {
		t.Fatalf("with 25-26第2学期 finalized, earned = %v, want 7 (T1+T2, never the future term)", got)
	}
	// 往回设一个更早的学期不该倒扣已修学分。
	if got := BuildStudentRecord("999999999999", agg, ctx, "24-25第1学期").Row["total_earned"]; got != float64(3) {
