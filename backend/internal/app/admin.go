...
func NewAdminServer(env Environment, config *ConfigStore, enrollment *EnrollmentService, live *LiveStudentService, syncRunner *SyncRunner, logger *slog.Logger) *AdminServer {
	return &AdminServer{env: env, config: config, enrollment: enrollment, live: live, syncRunner: syncRunner, logger: logger, sessions: map[string]adminSession{}}
}
...
func layoutHTML(title, body string, session *adminSession) string {
	nav := ""
	if session != nil {
		nav = `<nav><a href="/">总览</a><a href="/settings">日常设置</a><a href="/operations">任务</a><a href="/advanced">高级</a><form method="post" action="/logout"><input type="hidden" name="csrf" value="` + session.CSRF + `"><button class="link">退出</button></form></nav>`
	}
